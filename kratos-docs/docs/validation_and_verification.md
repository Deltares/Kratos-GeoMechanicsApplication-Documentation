...
Kratos GeoMechanics is validated through various unit tests, integration tests, and acceptance tests.

Each of them has a certain purpose, which are explain below. These tests are run either on every change in Kratos GeoMechanics or on a nightly basis, depending on the calculation time. 
...
Acceptance tests are physically the most realistic. These tests can be found on the [Kratos GeoMechanics website](https://github.com/KratosMultiphysics/Kratos/tree/master/applications/GeoMechanicsApplication/tests). Many of these tests have readable documentation available about the goal of this test and can also be used for reference to set up your own simulation.
